...
export function useCustomers(restaurantId?: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      console.log(
        "🔍 useCustomers - Fetching customers for restaurant:",
        restaurantId
      );

      // Gebruik de list API endpoint
      const response = await fetch(
        `/api/customers/list?restaurant_id=${restaurantId}`
      );
      ...
      console.log(
        "🔍 useCustomers - Returning customers:",
...
export function useSearchCustomers(restaurantId?: string, searchTerm?: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/customers/search?name=${encodeURIComponent(
          searchTerm
      ...
      console.log(
        "🔍 useSearchCustomers - Returning customers:",
        result.customers?.length || 0
      );
...
export function useCreateCustomer() {
  ...
  return useMutation({
    mutationFn: async (
      customer: Omit<
        Customer,
        "id" | "created_at" | "updated_at" | "total_visits" | "last_visit"
      >
    ) => {
